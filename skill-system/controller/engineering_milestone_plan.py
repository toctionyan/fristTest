...
def validate_plan(plan: Mapping[str, Any]) -> dict[str, Any]:
    ...
    for index, raw in enumerate(milestones, start=1):
        ...
        if order != index:
            raise EngineeringMilestoneError("milestone order must be contiguous and match list order")
        ...
        if not oracle or not runtime_test or not test_name:
            raise EngineeringMilestoneError(f"milestone {case_id} is missing executable evidence coordinates")
        normalized.append(
            {
                "id": case_id,
                "order": order,
    ...
    normalized_plan = {
        ...
        "required_product_gate": _text(plan.get("required_product_gate")) or "quality-quick-execution",
        "required_transport_gate": _text(plan.get("required_transport_gate")) or "quality-quick-required-status",
...
def bind_plan(
    ...
    *,
    ...
    binding = {
        ...
        "quality_run_attempt": _positive_int(quality_run_attempt, name="quality_run_attempt"),
...
def validate_binding(binding: Mapping[str, Any], *, current_head_sha: str | None = None) -> dict[str, Any]:
    ...
    if supplied.get("merge_allowed") is not False or supplied.get("deploy_allowed") is not False:
        raise EngineeringMilestoneError("milestone binding cannot grant merge or deploy authority")
    if supplied.get("production_closed") is not False or supplied.get("authority_effect") != "none":
    ...
    if current_head_sha is not None and _sha(current_head_sha, name="current_head_sha") != head:
        raise EngineeringMilestoneError("candidate head drifted from bound milestone plan")
...
def build_case_evidence(
    ...
    *,
    ...
    if normalized_status not in _ALLOWED_CASE_STATUS:
        raise EngineeringMilestoneError("case evidence must be an executed PASS or FAIL; skipped/pending is not certification")
    if not _text(evidence_ref) or not _text(test_name):
        raise EngineeringMilestoneError "case evidence requires durable evidence_ref and test_name")
    payload = {
...
def _validate_case_evidence(binding: Mapping[str, Any], evidence: Mapping[str, Any]) -> dict[str, Any]:
    bound = validate_binding(binding)
    ...
    if _text(raw.get("status")).upper() not in _ALLOWED_CASE_STATUS:
        raise EngineeringMilestoneError("case evidence status is not terminal executable evidence")
    result = dict(raw)
...
def certify_plan(
    ...
    *,
    ...
    if not _text(product_evidence_ref) or not _text(transport_evidence_ref):
        raise EngineeringMilestoneError("certification requires product and transport evidence refs")
    ...
    for raw in case_evidence:
        ...
        if case_id in observed:
            raise EngineeringMilestoneError(f"duplicate milestone evidence: {case_id}")
        if case_id not in bound["milestone_ids"]:
            raise EngineeringMilestoneError(f"unexpected milestone evidence: {case_id}")
        observed[case_id] = item

    missing = [case_id for case_id in bound["milestone_ids"] if case_id not in observed]
    failed = [case_id for case_id in bound["milestone_ids"] if observed.get(case_id, {}).get("status") == "FAIL"]

    if missing:
        decision = "INCOMPLETE_EVIDENCE"
        reason = "one or more required real-lifecycle milestones have no exact-head executable evidence"
    elif failed or product == "FAIL":
        decision = "PRODUCT_RED"
        reason = "real-lifecycle product evidence is red; transport state must not mask or rewrite product failure"
        ...
        reason = "all product milestones are green, but exact-head transport/publication certification is not green"
    else:
        decision = "CERTIFIED"
        reason = "all required product milestones and exact-head transport certification are green"
    ...
    result = {
        ...
        "passed_milestone_count": sum(1 for item in observed.values() if item.get("status") == "PASS"),
        ...
        "deploy_allowed": False,
        "production_closed": False,
        "evidence_refs": [_text(product_evidence_ref), _text(transport_evidence_ref)],
    }
