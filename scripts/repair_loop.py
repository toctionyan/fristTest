...
def _load_compatible_summary(evidence_dir: Path, *, workspace_fingerprint: str) -> dict[str, Any] | None:
    ...
    if recorded and recorded != workspace_fingerprint:
        return None
    return payload
...
def _run_judge_resilient(
    *,
    ...
    existing = _load_compatible_summary(
        evidence_dir,
        workspace_fingerprint=workspace_fingerprint,
    )
    if existing is not None:
        summary_path = _summary_path(evidence_dir)
        # Recovery is an idempotent read of durable evidence, not another Judge
        # execution.  This fast path prevents a crash after Judge output from
        # consuming the next execution budget or rerunning a read-only gate.
        task_run.set_metadata(
            current_summary_path=_evidence_ref(workspace, summary_path),
            current_evidence_dir=_evidence_ref(workspace, evidence_dir),
            recovered_existing_summary=True,
        )
        return 0, existing, ("recover-existing-summary",), summary_path
...
def _mark_existing_source_change(
    ...
    *,
    ...
    violations = quality_loop._scope_violations(
        workspace,
    ...
    if violations:
        ...
        raise ValueError(
            "interrupted fixer changed paths outside the frozen target: "
            + preview
            + suffix
        )
...
def main() -> int:
    ...
    try:
        ...
        return _block_and_return(
            task_run,
            workspace=workspace,
            code="INTERRUPTED_FIXER_RECONCILIATION_REJECTED",
            reason=str(exc),
            attempted_strategies=("baseline-diff-reconciliation",),
            next_action="restore out-of-scope drift or create a new governed repair target",
            evidence_refs=(_evidence_ref(workspace, target),),
            exit_code=7,
        )
    ...
    for cycle in range(1, min(args.max_cycles, quality_loop.MAX_REPAIR_ROUNDS) + 1):
        ...
        task_run.set_metadata(
            ...
            fix_pending_validation=summary.get("loop_status") == "CONVERGED",
