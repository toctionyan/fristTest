#!/usr/bin/env python3
"""Fail-closed validator for the B30 WP-02 implementation plan."""
...
EXPECTED_BASELINE = "e7e902c3363d867b8bea18ab2c76c0739252daad"
EXPECTED_CONTRACT_BLOB = "bcc9f2847de265fe207a6635647da2c492620eaa"
...
def validate(plan_path: Path, doc_path: Path) -> None:
    ...
    if plan.get("schema_version") != 1 or plan.get("stage") != "B30" or plan.get("work_package") != "WP-02":
    ...
    if not isinstance(plan.get("review_questions"), list) or len(plan["review_questions"]) < 7:
        raise PlanError("review_questions_incomplete")
    if not isinstance(plan.get("permit_preconditions"), list) or len(plan["permit_preconditions"]) < 6:
...
def main() -> int:
    ...
    parser.add_argument("--plan", type=Path, default=Path("governance/architecture/b30-wp02-implementation-plan.json"))
    parser.add_argument("--doc", type=Path, default=Path("docs/architecture/B30_WP02_IMPLEMENTATION_PLAN.md"))
    ...
    try:
        ...
        return 1
    print(json.dumps({"status": "PASS", "stage": "B30", "work_package": "WP-02", "artifact": "implementation_plan"}, ensure_ascii=False))
    return 0
