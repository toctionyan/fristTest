...
def test_resume_summary_requires_exact_workspace_fingerprint(tmp_path: Path) -> None:
    repair_loop = _load_script("repair_loop.py")
    evidence = tmp_path / "evidence"
    evidence.mkdir()
    summary_path = evidence / "run-summary.json"

    summary_path.write_text(json.dumps({"decision": "PASS"}), encoding="utf-8")
    assert repair_loop._load_compatible_summary(
        evidence, workspace_fingerprint="workspace-a"
    ) is None

    summary_path.write_text(
        json.dumps({
            "decision": "PASS",
            "workspace_snapshot_fingerprint": "workspace-b",
        }),
        encoding="utf-8",
    )
    assert repair_loop._load_compatible_summary(
    ...
    loaded = repair_loop._load_compatible_summary(
        evidence, workspace_fingerprint="workspace-a"
    )
