...
def _safe_file(raw: object, label: str) -> Path:
...
def verify() -> list[str]:
    ...
    if contract.target_kind.value in {"repair", "migration", "revert", "certification"}:
        try:
            target = _safe_file(quality_target, "quality_target")
            ...
            if not matches:
                errors.append("quality_target_scope_does_not_match_contract:" + json.dumps(details, ensure_ascii=False, sort_keys=True))
        except ValueError as exc:
            errors.append(str(exc))
    if contract.target_kind.value in {"repair", "migration", "revert"} and contract.status in {"implementing", "review", "verified", "closed"}:
