...
TEMP_FILES = {
    "scripts/apply_production_dispatch_manifest.py",
    ".github/workflows/apply-production-dispatch-manifest.yml",
}
REQUIRED_FILES = {
...
def main() -> None:
    ...
    if missing:
        raise RuntimeError(f"production dispatch files missing: {missing}")
    ...
    if not REQUIRED_FILES <= present:
        raise RuntimeError(
            f"manifest omitted production dispatch files: {sorted(REQUIRED_FILES - present)}"
        )
