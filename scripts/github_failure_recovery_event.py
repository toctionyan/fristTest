#!/usr/bin/env python3
"""Build a bounded workflow_run event for owner-authorized CI recovery.

The recovery lane is intentionally narrow. It accepts only an exact numeric command,
binds the requested failed run to the current open pull request and same repository,
and writes a synthetic event consumed by the existing Stage-1 ingestion controller.
It never reads Secrets, modifies source, or authorizes repair by itself.
"""
...
COMMAND = re.compile(r"^/governed-repair-run[ \t]+([1-9][0-9]*)[ \t]*$")
...
def parse_command(comment: str) -> int:
    ...
    if not match:
        raise RecoveryEventError(
            "comment must be exactly '/governed-repair-run <numeric-run-id>'"
        )
...
def build_event(
    *,
    ...
    if not isinstance(head_repository, dict) or _repo_name(head_repository.get("full_name")) != repo:
    ...
    if not isinstance(pr_head_repo, dict) or _repo_name(pr_head_repo.get("full_name")) != repo:
    ...
    return {
        ...
        "workflow_run": run,
        "recovery": {
            "schema": "owner-comment-recovery@1",
