...
runpy.run_path(str(Path(__file__).with_name("wp08-new-release-attempt4-repair-publisher-v2.py")), run_name="__main__")
...
if text.count(old_validator) != 1:
    raise SystemExit(f"expected generated validator once, found {text.count(old_validator)}")
...
if text.count(old_helper) != 1:
    raise SystemExit(f"expected generated helper evaluation once, found {text.count(old_helper)}")
smoke.write_text(text.replace(old_helper, new_helper, 1), encoding="utf-8")
