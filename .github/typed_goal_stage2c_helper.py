...
TEST_PATH = ROOT / "services/agent-service/tests/runtime/test_typed_goal_dependency_authority_shadow.py"
...
def apply() -> None:
    ...
    helper = '''def _typed_dependency_authority_shadow(
    *,
    plan: dict[str, Any],
...
'''
    ...
    if TEST_PATH.exists():
        raise SystemExit("typed_dependency_shadow_test_already_exists")
...
def refresh_baseline() -> None:
    ...
    if int(payload.get("file_count") or 0) != 582 or len(previous) != 582:
        raise SystemExit(
    ...
    expected_added = {
        "services/agent-service/tests/runtime/test_typed_goal_dependency_authority_shadow.py",
    }
...
def main() -> None:
    if len(sys.argv) != 2 or sys.argv[1] not in {"apply", "refresh-baseline"}:
        raise SystemExit("usage: typed_goal_stage2c_helper.py apply|refresh-baseline")
    if sys.argv[1] == "apply":
        apply()
    else:
        refresh_baseline()
