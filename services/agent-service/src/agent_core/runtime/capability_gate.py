...
def _pretool_frontier_proof(
    *,
    ...
    errors: list[str] = []
    allowed_tools = {str(value) for value in list(policy.get("allowed_capability_tools") or []) if str(value)}
    ...
    raw_goal_policies = [row for row in list(policy.get("goal_policies") or []) if isinstance(row, dict) and str(row.get("goal_id") or "")]
    goal_policy_by_id = {str(row.get("goal_id") or ""): row for row in raw_goal_policies}
    ...
    if goal_ids and raw_goal_policies:
        for goal_id in sorted(goal_ids):
            ...
            row_allowed_tools = {str(value) for value in list((row or {}).get("allowed_tools") or []) if str(value)}
    ...
    shared_binding_check: dict[str, Any] = {"required": len(goal_ids) > 1, "declared": False, "coverage_id": None, "coverage_proofs": {}}
    if len(goal_ids) > 1:
        expected_goal_ids = set(goal_ids)
        formal_goal_by_id = {str(goal.get("goal_id") or ""): goal for goal in semantic_goals(state) if str(goal.get("goal_id") or "")}
        ...
        matching_shared = next((
            row for row in list(policy.get("shared_frontier_bindings") or [])
        ...
        if matching_shared is None:
            ...
            coverage_proofs = matching_shared.get("coverage_proofs") if isinstance(matching_shared.get("coverage_proofs"), dict) else {}
            if set(coverage_proofs) != expected_goal_ids or any(not str((coverage_proofs.get(goal_id) or {}).get("output_name") or "") for goal_id in expected_goal_ids):
                errors.append("multi_goal_binding_completion_proof_incomplete")
            contract = capability_registry.contract_for_tool(tool_name)
            planning = contract.planning_contract if contract is not None and contract.contract_version == "2" and contract.planning_contract is not None else None
            primary_output_name = str(planning.completion.output_name or "") if planning is not None and planning.completion.mode == "tool_output" else ""
            primary_output = next((output for output in planning.produces if output.name == primary_output_name), None) if planning is not None and primary_output_name else None
            formal_by_id = {str(goal.get("goal_id") or ""): goal for goal in semantic_goals(state) if str(goal.get("goal_id") or "")}
            if primary_output is None or not primary_output.completion_proof:
                ...
                for goal_id in sorted(expected_goal_ids):
                    ...
                    expected_effect = canonical_effect_identity((formal_by_id.get(goal_id) or {}).get("requested_effect"))
                    if (
            ...
            shared_binding_check = {
                "required": True,
                "declared": not any(error.startswith("multi_goal_binding_") for error in errors),
...
def _parameterization_proof(state: dict[str, Any], args: dict[str, Any]) -> dict[str, Any]:
    ...
    for span_key, span_value in target.items():
        ...
        if not any(str(row.get("parameter_path") or "") == f"target.{field}" and str(row.get("source_span") or "") == span for row in rows):
            rows.append({
                "kind": "scope",
...
def _frozen_condition_operands(expression: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    if op in {"and", "or", "not"}:
        return [operand for child in list(expression.get("args") or []) if isinstance(child, dict) for operand in _frozen_condition_operands(child)]
    return [dict(expression[key]) for key in ("left", "right", "lower", "upper") if isinstance(expression.get(key), dict)]
    ...
    for goal_id in sorted(goal_ids):
        ...
        for operand in _frozen_condition_operands(condition):
            ...
            if key in seen:
                continue
            seen.add(key)
            requirements.append({"goal_id": goal_id, "operand_source": source, "condition_path": path, "parameter_leaf": path.rsplit(".", 1)[-1]})
    covered_bindings = [dict(row) for row in list(parameterization.get("bindings") or []) if isinstance(row, dict) and str(row.get("status") or "") == "covered"]
    checks: list[dict[str, Any]] = []
    errors: list[str] = []
    for requirement in requirements:
        leaf = str(requirement["parameter_leaf"])
        matches = [row for row in covered_bindings if str(row.get("parameter_path") or "").rsplit(".", 1)[-1] == leaf]
        if len(matches) == 1:
            checks.append({**requirement, "status": "covered", "matched_parameter_path": str(matches[0].get("parameter_path") or "")})
            continue
        code = f"formal_goal_condition_unbound:{requirement['goal_id']}:{requirement['condition_path']}" if not matches else f"formal_goal_condition_ambiguous:{requirement['goal_id']}:{requirement['condition_path']}"
        errors.append(code)
        checks.append({**requirement, "status": "uncovered", "matched_parameter_paths": [str(row.get("parameter_path") or "") for row in matches]})
    return {
...
def _formal_goal_scope_coverage_proof(
    ...
    *,
    ...
    for goal_id in sorted(goal_ids):
        goal = formal_by_id.get(goal_id) or {}
        target_candidate = goal.get("target_candidate") if isinstance(goal.get("target_candidate"), dict) else {}
    ...
    structural_leaves = {"mode", "operator", "left_handle", "right_handle", "source_handle", "reference_kind", "group_size", "expected_shape", "action", "capability"}
    ...
    for raw in list(parameterization.get("bindings") or []):
        ...
        if provenance == "runtime_target_evidence":
            ...
            eligible = provenance == "candidate_constraint_binding" and (path.startswith("query.") or path.startswith("target.")) and leaf not in structural_leaves and not leaf.endswith("_span")
    ...
    for requirement in requirements:
        ...
        parameter_matches = [row for row in candidate_rows if _literal_scope_overlap(formal_span, str(row.get("source_span") or ""))]
        lineage_matches = sorted(span for span in lineage_spans if _literal_scope_overlap(formal_span, span))
        covered = bool(parameter_matches or lineage_matches)
        if not covered:
            errors.append(f"formal_goal_scope_constraint_unbound:{requirement['goal_id']}:{requirement['scope_index']}")
        checks.append({**requirement, "status": "covered" if covered else "uncovered", "parameter_matches": parameter_matches, "verified_lineage_spans": lineage_matches})
    return {
...
def _visible_reference_proof(
    ...
    *,
    ...
    visible_refs = visible_result_refs_from_ledger(state.get("artifact_ledger") or [], state=state, limit=12)
    latest_refs = [ref for ref in visible_refs if bool(ref.get("is_latest_visible_turn")) and str(ref.get("result_ref") or "")]
    latest_handles = {str(ref.get("result_ref") or "") for ref in latest_refs}
    latest_scopes = {visible_result_scope_key(ref) for ref in latest_refs if visible_result_scope_key(ref) != ("empty", ())}
    latest_member_handles = {str(member) for ref in latest_refs for member in list(ref.get("member_handles") or []) if str(member)}
    ...
    selected_ranks = {visible_rank_by_handle[handle] for handle in selected_visible_handles if handle in visible_rank_by_handle}
    max_selected_rank = max(selected_ranks, default=0)
    required_prefix_handles = {handle for handle, rank in visible_rank_by_handle.items() if max_selected_rank and rank <= max_selected_rank}
    group_source_span = binding_span if binding_kind == "explicit_group_reference" else str(args.get("reference_span") or "").strip()
    explicit_group_binding = bool(
        ...
        and (binding_kind != "explicit_group_reference" or binding_group_size == len(selected_visible_handles))
    )
    ...
    if binding_kind == "explicit_return" and selected_visible_handles:
        ...
        selected_ref = next((ref for ref in visible_refs if str(ref.get("result_ref") or "") == selected_handle), {})
        labels = [str(value) for value in list(selected_ref.get("member_labels") or []) if str(value)]
        ...
        if not binding_key or not any(binding_key in label_key or label_key in binding_key for label_key in label_keys if label_key):
...
def _semantic_reference_binding_proof(
    ...
    *,
    ...
    target = args.get("target") if isinstance(args.get("target"), dict) else {}
    mode = str(target.get("mode") or "")
    actual_handles = {str(target.get(name) or "").strip() for name in ("left_handle", "right_handle", "source_handle") if str(target.get(name) or "").strip()}
    checks: list[dict[str, Any]] = []
    errors: list[str] = []
    ...
    for goal_id in sorted(goal_ids):
        ...
        if resolved is None and proof is None:
            checks.append({"goal_id": goal_id, "required": False, "matched": True, "reason_code": "no_frozen_historical_reference"})
            continue
        result_ref = str((resolved or {}).get("result_ref") or "").strip()
        member_handles = {str(value).strip() for value in list((resolved or {}).get("member_handles") or []) if str(value).strip()}
        ...
        if resolution_status != "UNIQUE" or not resolved:
        ...
        elif reference_cardinality == "collection" and goal_result_cardinality == "single" and len(member_handles) == 1:
            ...
            if result_ref and result_ref in actual_handles:
                ...
                reason = "resolved_singleton_collection_member_bound" if matched else "resolved_singleton_collection_target_mismatch"
        else:
...
def _literal_operation_in_text(operation_value: Any, user_text: Any) -> bool:
    ...
    return any(operation[start : start + minimum] in current for start in range(0, len(operation) - minimum + 1))
...
def _explicit_member_scope_proof(
    ...
    if target_mode == "entity_match":
        return {"version": "explicit-member-scope-proof@2", "complete": True, "applies": False, "errors": []}
    binding = args.get("context_binding") if isinstance(args.get("context_binding"), dict) else {}
    literal_spans = list(dict.fromkeys(str(value).strip() for value in (target.get("attribute_span"), args.get("reference_span"), binding.get("source_span")) if str(value or "").strip()))
    visible_members: dict[str, set[str]] = {}
    visible_labels: dict[str, str] = {}
    for ref in visible_result_refs_from_ledger(state.get("artifact_ledger") or [], state=state, limit=12):
    ...
    for span in literal_spans:
        ...
        handles = [
            ...
            if any(alias in literal or literal in alias for alias in aliases for literal in literal_aliases if literal)
        ]
        named_handles.update(handles)
        if handles:
            matches.append({"source_span": span, "matched_member_handles": handles, "matched_member_labels": [visible_labels.get(handle, handle) for handle in handles]})
    if len(named_handles) != 1:
        return {"version": "explicit-member-scope-proof@2", "complete": True, "applies": bool(matches), "literal_matches": matches, "errors": []}
    named_handle = next(iter(named_handles))
    right_ref = next((
        check.get("validated_ref") for check in list(visible_reference.get("checks") or [])
        if isinstance(check, dict) and str(check.get("parameter_path") or "") == "target.right_handle" and isinstance(check.get("validated_ref"), dict)
    ), None)
    right_members = {str(value) for value in list((right_ref or {}).get("member_handles") or []) if str(value)}
    if target_mode == "set_operation" and str(target.get("operator") or "") == "difference" and right_members == {named_handle}:
        return {
    ...
    selected_ref_members = [str(value) for value in list((selected_ref or {}).get("member_handles") or []) if str(value)]
    matched_indexes = [index for index, handle in enumerate(selected_ref_members) if handle == named_handle]
    mismatch = target_mode == "all_orders" or not selected_handles or selected_handles != {named_handle}
    error = "explicit_unique_member_target_mismatch" if selected_handles and named_handle not in selected_handles else "explicit_unique_member_requires_single_member_target"
    return {
        "version": "explicit-member-scope-proof@2",
...
def _derived_collection_scope_proof(args: dict[str, Any], visible_reference: dict[str, Any]) -> dict[str, Any]:
    target = args.get("target") if isinstance(args.get("target"), dict) else {}
    mode = str(target.get("mode") or "")
    ...
    if not legacy_sort and not (mode == "pipeline" and pipeline_sorts):
        return {"version": "derived-collection-scope-proof@1", "complete": True, "applies": False, "errors": []}
    left_handle = str(target.get("left_handle") or target.get("source_handle") or "")
    selected_ref = next((
        check.get("validated_ref")
        for check in list(visible_reference.get("checks") or [])
        if isinstance(check, dict) and str(check.get("result_ref") or "") == left_handle and isinstance(check.get("validated_ref"), dict)
    ), None)
    ...
    derived_selection = str(source.get("operator") or "") in {"sort", "take", "ordinal"} or any(str(row.get("op") or "") in {"sort", "take", "ordinal"} for row in source_steps)
    ...
    return {
        ...
        "requested_operation": ({key: target.get(key) for key in ("operator", "sort_field", "sort_direction", "sort_span") if target.get(key) not in (None, "")} if legacy_sort else {"mode": "pipeline", "steps": pipeline_steps}),
...
def issue_execution_permit(
    *,
    ...
    member_scope = (
        ...
        else {"version": "explicit-member-scope-proof@2", "complete": False, "applies": False, "errors": ["visible_result_reference_invalid"]}
    )
    derived_scope = (
        _derived_collection_scope_proof(normalized_args, visible_reference)
        if contract is not None and not arg_errors and visible_reference.get("complete")
        else {"version": "derived-collection-scope-proof@1", "complete": False, "applies": False, "errors": ["visible_result_reference_invalid"]}
    )
    formal_condition_coverage = _formal_goal_condition_coverage_proof(state, goal_ids=goal_ids, parameterization=parameterization)
    formal_scope_coverage = _formal_goal_scope_coverage_proof(state, goal_ids=goal_ids, parameterization=parameterization, visible_reference=visible_reference)
    semantic = (
    ...
    surface_rows = [row for row in list((surface or {}).get("goals") or []) if isinstance(row, dict) and str(row.get("goal_id") or "") in goal_ids]
    surface_required = surface is not None and bool(goal_ids)
    surface_allowed = not surface_required or bool(surface_rows) and all(tool_name in {str(value) for value in list(row.get("candidate_tools") or [])} for row in surface_rows)
    surface_proof = {
        "required": surface_required,
        "allowed": surface_allowed,
        "goal_ids": sorted(goal_ids),
        "candidate_tools": sorted({str(value) for row in surface_rows for value in list(row.get("candidate_tools") or []) if str(value)}),
    }
    frontier_proof = _pretool_frontier_proof(state=state, tool_name=tool_name, goal_ids=goal_ids, capability_registry=capability_registry)
    frontier_allowed = bool(frontier_proof.get("allowed"))
    formal_goals = semantic_goals(state)
    formal_effect_required = bool(
        formal_goals and goal_ids and all(
    ...
    formal_effect_proof = goal_effect_match_proof(state=state, tool_name=tool_name, goal_ids=sorted(goal_ids), registry=capability_registry)
    formal_effect_allowed = not formal_effect_required or bool(formal_effect_proof.get("allowed"))
    proof: dict[str, Any] = {
    ...
    if contract is None or arg_errors or not parameterization.get("parameterization_complete") or not formal_condition_coverage.get("complete") or not formal_scope_coverage.get("complete") or not visible_reference.get("complete") or not semantic_reference_binding.get("complete") or not member_scope.get("complete") or not derived_scope.get("complete") or not semantic_exact or not surface_allowed or not frontier_allowed or not formal_effect_allowed:
        return PermitDecision(
            ...
            rejection={
                "code": (
                    "CAPABILITY_NOT_AVAILABLE_IN_GOAL_SURFACE" if not surface_allowed
                    else "CAPABILITY_NOT_IN_PRETOOL_FRONTIER" if not frontier_allowed and frontier_proof.get("reason_code") == "tool_not_in_current_pretool_frontier"
                    else "PRETOOL_EXECUTION_POLICY_INVALID" if not frontier_allowed
                    else "CAPABILITY_GOAL_EFFECT_MISMATCH" if not formal_effect_allowed
                    else "SEMANTIC_REFERENCE_BINDING_MISMATCH" if contract is not None and not arg_errors and visible_reference.get("complete") and not semantic_reference_binding.get("complete")
                    else "DERIVED_SINGLETON_REQUIRES_PARENT_SCOPE" if contract is not None and not arg_errors and visible_reference.get("complete") and semantic_reference_binding.get("complete") and member_scope.get("complete") and not derived_scope.get("complete")
                    else "EXPLICIT_MEMBER_REQUIRES_SINGLE_MEMBER_TARGET" if contract is not None and not arg_errors and visible_reference.get("complete") and semantic_reference_binding.get("complete") and not member_scope.get("complete")
                    else "VISIBLE_RESULT_REF_INVALID" if contract is not None and not arg_errors and not visible_reference.get("complete")
                    else "CAPABILITY_SCOPE_CONSTRAINT_UNBOUND" if contract is not None and not arg_errors and not formal_scope_coverage.get("complete")
                    else "CAPABILITY_PARAMETERIZATION_INCOMPLETE" if contract is not None and not arg_errors and (not parameterization.get("parameterization_complete") or not formal_condition_coverage.get("complete"))
                    else "CAPABILITY_SEMANTIC_CLARIFICATION_REQUIRED" if semantic is not None and semantic.verdict == "clarify"
                    else "CAPABILITY_UNAVAILABLE" if semantic is not None and semantic.verdict == "unsupported"
                    else "CAPABILITY_EXACT_MATCH_REQUIRED"
                ),
                "message": (
                    "当前能力不属于本轮目标发现得到的有限候选集合，系统不会绕过能力面调用相近工具。" if not surface_allowed
                    else "当前能力不属于本轮 Pre-tool Execution Policy 的可执行前沿；即使调用被重放或伪造，Capability Gate 也不会签发 ExecutionPermit。" if not frontier_allowed and frontier_proof.get("reason_code") == "tool_not_in_current_pretool_frontier"
                    else "当前 Pre-tool Execution Policy 已失效、与语义合同或能力注册表不一致；系统已按失败关闭处理。" if not frontier_allowed
                    else "当前工具不能精确完成或支持其绑定 Goal 的 requested_effect；系统不会用相似能力替代。" if not formal_effect_allowed
                    else "当前工具目标与冻结语义合同中的历史引用解析证明不一致；系统不会使用较新、相似或更宽的对象替代。" if contract is not None and not arg_errors and visible_reference.get("complete") and not semantic_reference_binding.get("complete")
                    else "当前候选把已由排序或截取得到的单项再次作为比较全集；请使用该结果记录的父集合引用完成本轮比较。" if contract is not None and not arg_errors and visible_reference.get("complete") and semantic_reference_binding.get("complete") and member_scope.get("complete") and not derived_scope.get("complete")
                    else "用户证据明确点名了可见集合中的唯一成员，系统拒绝用整个集合代替该成员；请改用该成员的精确可验证引用。" if contract is not None and not arg_errors and visible_reference.get("complete") and semantic_reference_binding.get("complete") and not member_scope.get("complete")
                    else "当前引用的结果不存在、未向用户展示、已失效或与所需对象形态不符；系统不会改选其他结果。" if contract is not None and not arg_errors and not visible_reference.get("complete")
                    else "当前请求中冻结的目标范围约束没有绑定到真实查询/目标参数或已验证结果血缘，系统不会用更宽泛查询代替。" if contract is not None and not arg_errors and not formal_scope_coverage.get("complete")
                    else "当前请求中的决定性条件没有被完整绑定到正式参数，系统不会用更宽泛查询代替。" if contract is not None and not arg_errors and (not parameterization.get("parameterization_complete") or not formal_condition_coverage.get("complete"))
                    else "当前请求需要先澄清，系统不会自行改用相近能力。" if semantic is not None and semantic.verdict == "clarify"
                    else "当前系统没有与该请求精确匹配的能力，未改用相近工具。" if semantic is not None and semantic.verdict == "unsupported"
    ...
    return PermitDecision(permitted=True, match_proof=proof, execution_permit=permit, normalized_arguments=normalized_args)
...
def build_effects(*, plan_id: str, calls: list[dict[str, Any]], capability_registry: CapabilityRegistry, existing_effects: list[dict[str, Any]] | None = None) -> tuple[list[dict[str, Any]], list[dict[str, Any]]]:
    ...
    for index, raw_call in enumerate(calls):
        ...
        goal_ids = list(dict.fromkeys(str(value).strip() for value in list(args.pop("goal_ids", []) or []) if str(value).strip()))
        call["args"] = args
        ...
        effect = {
            ...
            "completion_effect_identities": list(completion_effects_for_contract(contract) if contract else ()),
            "support_effect_identities": list(support_effects_for_contract(contract) if contract else ()),
