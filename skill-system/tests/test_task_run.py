...
class TaskRunStoreTest(unittest.TestCase):
    ...
    def test_block_persists_blocker_and_checkpoint_in_one_revision(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            store = self._store(Path(tmp))
            revision = store.payload["revision"]
            store.block(
                code="TEST_BLOCKER",
                reason="prove atomic blocker persistence",
                attempted_strategies=("primary",),
                next_action="repair the external dependency",
                workspace_fingerprint="workspace-a",
                evidence_refs=("evidence/blocker.json",),
            )
            self.assertEqual(store.payload["revision"], revision + 1)
            self.assertEqual(store.payload["status"], "BLOCKED")
            self.assertEqual(store.payload["blockers"][-1]["code"], "TEST_BLOCKER")
            self.assertEqual(store.payload["checkpoints"][-1]["phase"], "TEST_BLOCKER")

    def test_completion_requires_final_evidence_even_when_conditions_are_satisfied(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            store = self._store(Path(tmp))
            store.checkpoint(
                status="RUNNING",
            ...
            for name in store.payload["required_conditions"]:
                store.mark_condition(name, evidence_refs=(f"evidence/{name}.json",))
            with self.assertRaises(PrematureCompletionError):
                store.complete(workspace_fingerprint="workspace-a", evidence_refs=())
    ...
    def test_persisted_completed_state_without_condition_evidence_is_rejected(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            store = self._store(root)
            payload = json.loads(store.path.read_text(encoding="utf-8"))
            ...
            payload["checkpoints"].append({
                "sequence": len(payload["checkpoints"]) + 1,
                "status": "COMPLETED",
                "phase": "COMPLETED",
                "workspace_fingerprint": "workspace-a",
                "evidence_refs": ["evidence/final.json"],
    ...
    def test_persisted_completed_state_without_final_checkpoint_evidence_is_rejected(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            store = self._store(root)
            store.checkpoint(
            ...
            payload = json.loads(store.path.read_text(encoding="utf-8"))
            payload["checkpoints"][-1]["evidence_refs"] = []
            store.path.write_text(json.dumps(payload), encoding="utf-8")
            with self.assertRaises(TaskRunError):
                TaskRunStore(store.path, payload)
