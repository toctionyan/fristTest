...
def _resolve_transaction_execution(value: TransactionExecutionDeps | None) -> TransactionExecutionDeps:
    """Compatibility seam for direct node tests; production composition passes deps."""
    return value or TransactionExecutionDeps(business_port=get_business_port(), outcome_factory=outcome)
...
def _append_formal_disposition(
    state: dict[str, Any], patch: dict[str, Any], *, tool_name: str, tool_signature: str, result: dict[str, Any],
) -> dict[str, Any]:
    """Attach the closed post-execution classification to a formal runtime step."""
    disposition = classify_execution_disposition(state=state, tool_name=tool_name, tool_signature=tool_signature, result=result)
    ...
    if trace and isinstance(trace[-1], dict) and str(trace[-1].get("name") or "") == tool_name:
        latest = dict(trace[-1]); latest_result = dict(latest.get("result") or {})
        latest_result.update(result); latest_result["execution_disposition"] = disposition
        latest["result"] = latest_result; trace[-1] = latest
    return {
        **patch,
        "tool_trace": trace,
        ...
        "decision_chain": _append_decision(
            {**state, **patch}, stage="classify_formal_transaction_execution", decision="execution_disposition_recorded",
            details={"tool_name": tool_name, "disposition": disposition.get("disposition"), "runtime_action": disposition.get("runtime_action"), "auto_target_switch": False},
        ),
    }


def action_gateway_node(state: dict[str, Any], *, transaction_execution: TransactionExecutionDeps | None = None) -> dict[str, Any]:
    ...
    business_conclusion = decision in {"rejected", "human_review", "review_required"}; clarification = decision == "clarify"
    if business_conclusion:
        runtime = outcome("preview_rejected", effects="none", safe_to_continue=False, correlation_id=str(state.get("correlation_id") or "") or None, customer_safe_summary=reason, next_interaction="none", payload={"gateway_decision": decision, **gateway}).as_dict(); code = "GATEWAY_BUSINESS_CONCLUSION"
    elif clarification:
        runtime = outcome("clarification", effects="none", safe_to_continue=False, correlation_id=str(state.get("correlation_id") or "") or None, customer_safe_summary=reason, next_interaction="need_selection", payload={"gateway_decision": decision, **gateway}).as_dict(); code = "CAPABILITY_SEMANTIC_CLARIFICATION_REQUIRED"
    else:
        runtime = outcome(
            ...
            safe_to_continue=True, correlation_id=str(state.get("correlation_id") or "") or None,
            evidence_handles=[str(gateway.get("offer_handle") or "")], customer_safe_summary=reason,
        ...
        ).as_dict(); code = "GATEWAY_PROGRESS"
    result = {"ok": not business_conclusion, "code": code, "message": reason, "data": {**gateway, "business_conclusion": business_conclusion}, "runtime_outcome": runtime}
    signature = "action_gateway:" + str(gateway.get("offer_handle") or "none") + ":" + decision
    return _append_formal_disposition(state, {**patch, "runtime_outcome": runtime}, tool_name="action_gateway", tool_signature=signature, result=result)


def action_confirmation_node(state: dict[str, Any], *, transaction_execution: TransactionExecutionDeps | None = None) -> dict[str, Any]:
...
def commit_action_node(state: dict[str, Any], *, transaction_execution: TransactionExecutionDeps | None = None) -> dict[str, Any]:
    ...
    decision = str(gateway.get("decision") or ""); status = str(patch.get("status") or "")
    code = "SUBMISSION_UNKNOWN" if decision == "submission_unknown" or status == "ActionSubmissionUnknown" else "COMMIT_EXECUTED"
    result = {
        "ok": decision == "committed", "code": code,
        "message": str(gateway.get("message") or patch.get("current_final_answer") or "提交状态已更新。"),
        "data": {**gateway, "business_conclusion": decision in {"committed", "commit_failed"} or status in {"ActionUnavailable", "ActionCapabilityReviewRequired", "ActionTargetUnavailable", "ActionCommitPreflightRejected", "ActionCommitEnvelopeInvalid", "ActionAuthorityInvalid"}},
    ...
    return _append_formal_disposition(state, patch, tool_name="commit_action", tool_signature=signature, result=result)


def _execute_business_command_envelope(
    state: dict[str, Any], envelope: dict[str, Any], *, idempotency_key: str, transaction_execution: TransactionExecutionDeps | None = None,
) -> dict[str, Any]:
    """Compatibility facade; production graph passes explicit dependencies."""
    deps = transaction_execution or TransactionExecutionDeps(business_port=get_business_port(), outcome_factory=outcome)
    return _transaction_execute_business_command_envelope(state, envelope, idempotency_key=idempotency_key, deps=deps)


def reconcile_submission_node(state: dict[str, Any], *, transaction_execution: TransactionExecutionDeps | None = None) -> dict[str, Any]:
    ...
    patch = reconcile_attempts(
        state, store=store,
        execute_envelope=lambda current, envelope, idempotency_key: _execute_business_command_envelope(current, envelope, idempotency_key=idempotency_key) if transaction_execution is None else _execute_business_command_envelope(current, envelope, idempotency_key=idempotency_key, transaction_execution=transaction_execution),
    ...
    if patch is None:
        ...
        result = {
            "ok": True, "code": "RECONCILIATION_NOT_REQUIRED", "message": answer,
            "data": {"business_conclusion": True, "reconciliation": []},
            "runtime_outcome": outcome("transaction_status", effects="none", safe_to_continue=False, correlation_id=str(state.get("correlation_id") or "") or None, customer_safe_summary=answer, next_interaction="none", payload={"reconciliation": []}).as_dict(),
        }
        final_patch = {
            "phase": "final", "status": "SubmissionReconciliationNoPendingAttempt", "current_final_answer": answer,
            "model_mode_restriction": ["explain"], "runtime_outcome": result["runtime_outcome"],
            "tool_trace": [*(state.get("tool_trace") or []), {"plan_id": str((state.get("current_turn_plan") or {}).get("plan_id") or ""), "loop_step": int(state.get("agent_loop_step") or 0), "name": "reconcile_submission", "args": {"mode": "same_idempotency_key_only"}, "result": result, "classification": "transaction_reconciliation"}],
        }
    else:
        reconciled = list(patch.get("transaction_reconciliation") or [])
        unknown = any(str(row.get("state") or "") in {"SUBMISSION_UNKNOWN", "RECONCILIATION_REQUIRED"} for row in reconciled if isinstance(row, dict))
        answer = "提交结果仍在业务系统对账中，系统未重复提交；请稍后查询办理状态。" if unknown else "已按原提交记录完成对账，未创建新的重复业务操作。"
        result = {
            "ok": not unknown, "code": "SUBMISSION_UNKNOWN" if unknown else "RECONCILIATION_COMPLETE", "message": answer,
            "data": {"business_conclusion": not unknown, "reconciliation": reconciled},
            "runtime_outcome": outcome("submission_unknown" if unknown else "transaction_status", effects="unknown" if unknown else "none", safe_to_continue=False, correlation_id=str(state.get("correlation_id") or "") or None, customer_safe_summary=answer, next_interaction="retry_later" if unknown else "show_status", payload={"reconciliation": reconciled}).as_dict(),
        }
        final_patch = {
            **patch, "phase": "final", "status": "SubmissionReconciliationPending" if unknown else "SubmissionReconciled",
            "current_final_answer": answer, "model_mode_restriction": ["explain"], "runtime_outcome": result["runtime_outcome"],
            "tool_trace": [*(patch.get("tool_trace") or state.get("tool_trace") or []), {"plan_id": str((state.get("current_turn_plan") or {}).get("plan_id") or ""), "loop_step": int(state.get("agent_loop_step") or 0), "name": "reconcile_submission", "args": {"mode": "same_idempotency_key_only"}, "result": result, "classification": "transaction_reconciliation"}],
        }
    return _append_formal_disposition(state, final_patch, tool_name="reconcile_submission", tool_signature="reconcile_submission:same_idempotency_key_only", result=result)


__all__ = [
    "COMMITTABLE_TRANSACTION_ACTION_IDS", "get_business_port", "prepare_agent_loop_turn_node", "build_context_bundle_node",
    "agent_loop_node", "execute_agent_loop_calls_node", "classify_execution_disposition_node", "route_after_execution_disposition",
    "action_gateway_node", "action_confirmation_node", "commit_action_node", "finalize_agent_loop_turn_node",
    "trim_agent_loop_messages_node", "persist_and_stream_node", "route_after_agent_loop", "route_after_confirmation", "reconcile_submission_node",
]
