...
if start < 0:
    raise SystemExit("first workflow resume-condition replacement is missing")
end = source.find("replace_once(\n    workflow_path,", start + len(marker))
if end < 0:
    raise SystemExit("next workflow replacement marker is missing")
replacement = '''workflow_text = workflow_path.read_text(encoding="utf-8")
