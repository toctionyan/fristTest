...
class EngineeringCIConvergenceWorkflowTests(unittest.TestCase):
    ...
    def test_is_exact_head_and_fail_closed(self) -> None:
        ...
        self.assertIn("--control-plane-ref", self.source)
        self.assertIn('.schema == "ci-convergence@2"', self.source)
        ...
        self.assertIn("actual=$(git -C control rev-parse HEAD)", self.source)
        convergence = (ROOT / "scripts" / "ci_convergence.py").read_text(encoding="utf-8")
        self.assertIn(".github/workflows/quality.yml", convergence)
        self.assertIn(".github/workflows/skill-self-validation.yml", convergence)
        self.assertIn("commits/${HEAD_SHA}/pulls?per_page=100", self.source)
        self.assertIn("expected_one_exact_open_pr", self.source)
        ...
        self.assertIn('id: reduce', self.source)
        self.assertIn("if: ${{ always() }}", self.source)
        self.assertIn("steps.reduce.outputs.publish == 'true'", self.source)
        self.assertIn("run_snapshot_found=false", self.source)
        self.assertIn("SOURCE_READY", self.source)
        self.assertIn("SOURCE_BLOCK_REASON", self.source)
        self.assertIn("SOURCE_STATUS_PUBLISH_ALLOWED", self.source)
        self.assertIn("--source-status", self.source)
        self.assertIn("control_plane_verification_failed", self.source)
        ...
        self.assertIn("Completed event is not terminal in the exact API snapshot; no status update published.", self.source)
        self.assertIn("No exact source evidence; no status update published.", self.source)
        self.assertNotIn('&& \"${EVENT_ACTION}\" != \"completed\"', self.source)
