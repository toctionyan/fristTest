...
bound Stage-2 patch to the exact failed source commit, verifies the resulting Git tree
matches the independently validated tree, and creates the local commit that may be
pushed to a governed repair branch. It never runs candidate tests or model code.
"""
...
def prepare_publication(
    *,
    ...
    expected_paths = tuple(_normalize(str(item)) for item in plan.get("changed_paths") or [])
    if not expected_paths or len(set(expected_paths)) != len(expected_paths):
        raise PublicationError("validated path set is empty or duplicated")
    check = _run(
    ...
    if set(actual_paths) != set(expected_paths) or len(actual_paths) != len(expected_paths):
        raise PublicationError(
            f"publisher path mismatch: expected={list(expected_paths)} actual={list(actual_paths)}"
        )

    _git(workspace, "add", "--", *expected_paths)
