...
9. 在模型调用前检查候选树必须干净，依赖安装或供应链脚本造成的任何未提交变更都会阻断任务；
10. 每个修复周期依次执行四个隔离职责：`failure-explorer` → `repair-plan-reviewer` → `restricted-fixer` → `diff-integrity-reviewer`。计划或 Diff 被独立 Reviewer 拒绝时不会应用 Patch；
11. 受限 Fixer 只能修改冻结的精确文件集合，不得新增、删除或重命名文件，不得修改治理、Judge、Bridge、Secret 或通过 skip/xfail/弱断言降低测试要求；
12. 每轮修复形成独立 Git Commit，再由不持有模型密钥的验证环境执行 Quick Quality Loop 和确定性 Integration Quality Loop；
13. 确定性 Integration 只排除两个受保护的真实模型浏览器 Gate，这两个 Gate 仍由 WP-08 生产认证负责；
14. 最多执行 8 轮；每轮最多 4 次治理模型调用。相同失败签名连续出现两次、无源码变化、范围漂移、候选树污染或预算耗尽时立即停止并保留证据；
15. 验证通过后推送 `governed-repair/*` 分支并创建 Draft PR，不直接写 `main`、不自动合并；
16. 创建 Draft PR 后显式 dispatch 正常 `quality` 和 `skill-self-validation` 检查。正常 Quality 的 Static/Quick 仍运行；Integration 使用内部 `skip` profile，避免重复运行已经由 Repair Job 完成的确定性 Integration；
17. TaskRun 只有在源码变更、独立 Quick+Integration 通过并发布 Draft PR 后才能完成。
...
- 失败候选代码以无持久凭据方式检出；失败 Artifact、日志、源码注释、前序模型输出和候选 Patch 都按不可信数据处理。
- 模型密钥只存在于四职责修复 Step；运行候选测试前会从环境中移除。
- 自动修复范围限于产品源码根，不能修改 `.github/`、`deployment/`、`scripts/`、`governance/`、`skill-system/`、Quality Judge、Repair Loop 或本桥接自身。
- 模型 Patch 必须通过精确 Allowlist、文件/行数预算、禁止新增删除、禁止弱化规则、独立 Diff Review 和 `git apply --check`。
- Python/pytest 缓存已显式忽略；任何其他未提交候选变化仍会触发候选树污染门禁。
...
正式启用后应使用隔离的 Canary 分支制造一个可逆的产品代码失败，验证以下证据链：失败 Run → Ingest Artifact/TaskRun → 四职责受限修复 → Quick+确定性 Integration → Draft PR → 正常 Status Checks。Canary PR 和分支不得合并到 `main`。
