...
def _validate_payload(payload: dict[str, Any]) -> None:
    ...
    if status == "COMPLETED":
        ...
        if not decision.eligible:
            raise TaskRunError(
                "persisted COMPLETED task-run lacks required completion evidence; "
        ...
        final_refs = latest.get("evidence_refs") if isinstance(latest, dict) else None
        if (
            not isinstance(latest, dict)
            or latest.get("status") != "COMPLETED"
            or latest.get("phase") != "COMPLETED"
            or not isinstance(final_refs, list)
            or not final_refs
            or any(not str(item).strip() for item in final_refs)
        ):
            raise TaskRunError(
                "persisted COMPLETED task-run requires a final COMPLETED checkpoint with evidence"
            )
...
class TaskRunStore:
    ...
    def block(
        ...
        *,
        ...
        # Persist the blocker and BLOCKED checkpoint in one atomic file replacement.
        # A crash must never leave a blocker record without the matching state.
    ...
    def complete(
        ...
        *,
        ...
        refs = [str(item) for item in evidence_refs if str(item).strip()]
        if not refs:
            raise PrematureCompletionError("task-run completion requires final evidence")
        self.checkpoint(
            status="COMPLETED",
            phase="COMPLETED",
            workspace_fingerprint=workspace_fingerprint,
